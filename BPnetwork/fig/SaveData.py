...
def run():
    ...
    # normalize images
    train_images = normalize(load_train_images().T)
    train_labels = load_train_labels().astype(int)
    test_images = normalize(load_test_images().T)
    test_labels = load_test_labels().astype(int)

    import pickle
    train = []
    train.append(train_images)
    train.append(train_labels)

    test = []
    test.append(test_images)
    test.append(test_labels)

    # test.append(train_images[50000:])
    # test.append(train_labels[50000:])

    # print(train[0].shape, train[1].shape)
    # print(test[0].shape, test[1].shape)

    output1 = open('data/train.pkl', 'wb')
    pickle.dump(train, output1)
    output1.close()

    output2 = open('data/test.pkl', 'wb')
    ...
    # import gzip
    # f_in = open('data.pkl', 'rb')
    # f_out = gzip.open('data.pkl.gz', 'wb')
